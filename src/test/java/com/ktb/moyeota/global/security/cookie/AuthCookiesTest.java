...
class AuthCookiesTest {
    ...
    private static final URI CALLBACK = URI.create(ORIGIN + "/auth/kakao/callback");
    ...
    @ParameterizedTest
    @ValueSource(strings = {"/users", "/users/nickname-availability", "/images/presigned-url"})
    ...
    void expiredSignupCookieRemovesIssuedOne() throws IOException {
        ...
        assertThat(cookieNamesSentTo(browser, "/images/presigned-url")).doesNotContain(AuthCookies.SIGNUP_TOKEN);
    }
    ...
    void refreshCookieStaysUnderAuth() throws IOException {
        ...
        assertThat(cookieNamesSentTo(browser, "/auth/tokens")).contains(AuthCookies.REFRESH_TOKEN);
        assertThat(cookieNamesSentTo(browser, "/images/presigned-url")).doesNotContain(AuthCookies.REFRESH_TOKEN);
    }
